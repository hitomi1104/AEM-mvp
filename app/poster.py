...
def submit_with_logging(payload, url):
    ...
    if result["status"] == "success":
        _log_status("SUCCESS", payload, "OK", code=result.get("code"))
    else:
        _log_status("FAILURE", payload, result.get("error", "Unknown error"), code=result.get("code"))
        _save_failed_payload(payload, error_msg=result.get("error", "Unknown error"))
...
def retry_failed_payloads(url, max_retries=3):
    """
    Legacy fallback to retry all failed payloads (mostly unused now).
    Runner script is preferred for real retry logic.
    """
    if not os.path.exists(FAILED_FILE):
        print("No failed submissions to retry.")
        return
    ...
    with open(FAILED_FILE, "r") as f:
        for line in f:
            ...
            try:
                ...
                if res["status"] == "success":
                    _log_status("RETRY SUCCESS", payload, "OK", code=res.get("code"))
                else:
                    _log_status("RETRY FAILURE", payload, res.get("error", "Unknown error"), code=res.get("code"))
                    if "payload" in obj:
                        obj["retry_count"] = retry_count + 1
                ...
                _log_status("RETRY ERROR", {"reportId": "N/A"}, f"Invalid JSONL line → {str(e)}")
